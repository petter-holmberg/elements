...
struct s
{
    e::array_double_ended<s> x;
};

SCENARIO ("Using double-ended array", "[array_double_ended]")
{
    e::array_double_ended<int> x{5};
    e::emplace(x, 0);
    e::emplace(x, 1);
    e::emplace(x, 2);
    e::emplace(x, 3);
    ...
    SECTION ("Monadic interface")
    {
        auto fn0 = [](int const& i){
            e::array_double_ended<int> ret{2};
            e::emplace(ret, i);
            e::emplace(ret, -i);
            return ret;
        };
