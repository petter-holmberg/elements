...
SCENARIO ("Distance", "[distance]")
{
    SECTION ("Calculating the distance between values under a transformation")
    {
        ...
        REQUIRE (e::power_unary(0, 0, inc_2) == 0);
        REQUIRE (e::power_unary(0, 1, inc_2) == 2);
        REQUIRE (e::power_unary(0, 2, inc_2) == 4);
        REQUIRE (e::distance(0, 0, inc_2) == 0);
        REQUIRE (e::distance(0, 10, inc_2) == 5);
    }

    SECTION ("Calculating the distance between values under an action")
    {
        auto inc_2 = [](int& x){ x = x + 2; };

        static_assert(e::Action<decltype(inc_2), int>);
        static_assert(e::Same_as<e::Distance_type<decltype(inc_2), int>, unsigned int>);
    ...
    SECTION ("Calculating orbit structure when a transformation is terminating")
    {
        auto inc_2 = [](int x){ return x + 2; };
    ...
    SECTION ("Calculating orbit structure when an action is terminating")
    {
        auto inc_2 = [](int& x){ x = x + 2; };
        auto is_limit = [](int x){ return x < 10; };
    ...
    SECTION ("Calculating orbit structure when a transformation is circular")
    {
    ...
    SECTION ("Calculating orbit structure when an action is circular")
    {
        auto inc_2_mod_10 = [](int& x){ x = (x + 2) % 10; };
        auto is_limit = [](int x){ return x < 10; };
    ...
    SECTION ("Calculating orbit structure when a transformation is rho-shaped")
    {
    ...
    SECTION ("Calculating orbit structure when an action is rho-shaped")
    {
        auto inc_2_rho = [](int& x){ x = (x == 10 ? 2 : x + 2); };
        auto is_limit = [](int x){ return x <= 10; };
