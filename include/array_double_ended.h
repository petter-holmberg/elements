...
template <typename T, Invocable auto alloc>
struct functor_t<array_double_ended<T, alloc>>
{
    using constructor_type = array_double_ended<T, alloc>;

    template <Operation<T> Op>
    static constexpr auto
    fmap(array_double_ended<T, alloc>& x, Op op) -> array_double_ended<T, alloc>&
    {
        using elements::copy;
    ...
    static constexpr auto
    fmap(array_double_ended<T, alloc>&& x, F fun) -> array_double_ended<Return_type<F, T>, alloc>
    {
        using elements::map;
        array_double_ended<Return_type<F, T>, alloc> y;
        reserve(y, size(x));
