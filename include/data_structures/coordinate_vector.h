...
template <
    ...
    typename S_add_op = sum<S>, typename S_mul_op = product<S>>
requires
    Field<S, S_add_op, S_mul_op> and
    Sequence<E>
...
{
    using type = Distance_type<Position_type<E>>;
...
requires Field<S, S_add_op, S_mul_op>
constexpr auto
is_empty(coordinate_vector<S, k, E, S_add_op, S_mul_op> const& x) -> bool
{
